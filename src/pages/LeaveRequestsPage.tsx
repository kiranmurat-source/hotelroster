...
import { Trash2, CalendarCheck, Calendar as CalendarIcon } from "lucide-react";
import { format } from "date-fns";
import { tr, enUS } from "date-fns/locale";

type LeaveType = "annual" | "compensatory" | "public_holiday" | "sick";
...
const leaveTypeConfig: Record<LeaveType, { label: string; labelEn: string; color: string }> = {
  ...
  compensatory: { label: "Alacak Off", labelEn: "Compensatory", color: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300" },
  public_holiday: { label: "Resmi Tatil", labelEn: "Public Holiday", color: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300" },
  sick: { label: "Hastalık İzni", labelEn: "Sick Leave", color: "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300" },
};
...
const LeaveRequestsPage = () => {
  ...
  const dateFnsLocale = language === "tr" ? tr : enUS;
  ...
  // Holiday apply modal
  const [applyHoliday, setApplyHoliday] = useState<PublicHoliday | null>(null);
  const [selectedDepts, setSelectedDepts] = useState<string[]>([]);

  // Load data
  useEffect(() => {
    const load = async () => {
      const [profilesRes, leavesRes, holidaysRes] = await Promise.all([
  ...
  const handleDelete = useCallback(async (id: string) => {
    try {
      // Delete associated roster shifts first
  ...
  const handleApplyHoliday = useCallback(async (holiday: PublicHoliday) => {
    if (selectedDepts.length === 0 || !user) {
      toast.error(t("leave.selectDepts"));
      return;
    }

    try {
      const staffInDepts = profiles.filter((p) => p.department && selectedDepts.includes(p.department));
      ...
      const rosterRows = staffInDepts.map((p) => ({
        user_id: user.id,
        staff_name: p.display_name || "Unknown",
        date: holiday.date,
        shift: "OFF",
      ...
      if (rosterRows.length > 0) {
        const { error } = await supabase.from("roster_shifts").insert(rosterRows);
        if (error) throw error;
      }

      toast.success(t("leave.holidayApplied").replace("{count}", String(staffInDepts.length)));
      setApplyHoliday(null);
      setSelectedDepts([]);
    } catch (err: any) {
      toast.error(err.message || "Error");
    }
  }, [selectedDepts, profiles, user, t]);
  ...
  return (
    <AppLayout>
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isUpcoming ? "bg-amber-50 dark:bg-amber-900/10 border border-amber-200/50" : "bg-muted/50"
                      )}
                    >
                      <div>
                        <p className={cn("font-medium", isUpcoming ? "text-amber-800 dark:text-amber-300" : "text-muted-foreground")}>
                          {h.name}
                        </p>
                      ...
                      {isManager && (
                        <Button
                          ...
                          onClick={() => { setApplyHoliday(h); setSelectedDepts([]); }}
                        >
                          {t("leave.applyAll")}
                        </Button>
                      )}
      ...
      {/* Apply Holiday Modal */}
      <Dialog open={!!applyHoliday} onOpenChange={() => setApplyHoliday(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("leave.applyHolidayTitle")}</DialogTitle>
          </DialogHeader>
          {applyHoliday && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                <strong>{applyHoliday.name}</strong> — {formatHolidayDate(applyHoliday.date)}
              </p>
              <p className="text-sm">{t("leave.selectDeptsLabel")}</p>
              <div className="space-y-2">
                ...
                {departments.length === 0 && (
                  <p className="text-xs text-muted-foreground">{t("leave.noDepts")}</p>
                )}
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setApplyHoliday(null)}>{t("leave.cancel")}</Button>
            <Button onClick={() => applyHoliday && handleApplyHoliday(applyHoliday)} disabled={selectedDepts.length === 0}>
              {t("leave.confirm")}
            </Button>
          </DialogFooter>
        </DialogContent>
